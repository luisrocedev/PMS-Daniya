...
if ($m === 'GET') {
    ...
    if ($id) {
        echo json_encode($super->getById('ausencias', $id));
        exit;
    }
    $sql = "SELECT * FROM ausencias $where ORDER BY fecha DESC";
    $st = $pdo->prepare($sql);
    ...
    $sql = "INSERT INTO ausencias (id_empleado,fecha,$campo) 
    ...
    echo json_encode($super->update('asuencias', $id, $data) ? ['success' => true] : ['error' => 'No se pudo actualizar']);
} elseif ($m === 'DELETE') {
    if (!$id) {
        echo json_encode(['error' => 'Falta id']);
        exit;
    }
    echo json_encode($super->delete('asuencias', $id) ? ['success' => true] : ['error' => 'No se pudo eliminar']);
